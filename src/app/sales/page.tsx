...
export default function SalesPage() {
  ...
  useEffect(() => {
    const closeCustomerSuggest = (e: MouseEvent | TouchEvent) => {
      if (customerBoxRef.current && !customerBoxRef.current.contains(e.target as Node)) setCustomerSuggestOpen(false);
    };
    document.addEventListener("mousedown", closeCustomerSuggest);
    document.addEventListener("touchstart", closeCustomerSuggest);
    return () => {
      document.removeEventListener("mousedown", closeCustomerSuggest);
      document.removeEventListener("touchstart", closeCustomerSuggest);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`/api/sales/customers?q=${encodeURIComponent(search)}`, { cache: "no-store" });
        const data = await res.json();
        if (!active) return;
  ...
  const resetAfterSave = async () => {
    setLines([]); setAmountCollected(0); setCustomerName(""); setCustomerPhone(""); setCustomerSuggestions([]); setCustomerSuggestOpen(false); setCommission(settings.defaultCommission || 0); setCollaboratorName(""); setDefaultWarrantyMonths(settings.defaultWarrantyMonths || 0); setPaymentType(settings.defaultPaymentType || "CASH"); setFreeShip((settings.defaultShippingCost || 0) > 0); setFreeShipCost(settings.defaultShippingCost || 0); setGiftAccessory((settings.defaultAccessoryCost || 0) > 0); setGiftAccessoryCost(settings.defaultAccessoryCost || 0); setOtherExtraCost(settings.defaultOtherCost || 0); setNote("");
  ...
  return <div style={{ display: "grid", gap: 12 }}>
    <section className="panel" style={{ padding: 12, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.35fr 1fr .9fr", gap: 12, alignItems: "start" }}>
      ...
      <div ref={customerBoxRef} className="panel" style={{ padding: 12, background: "#f8fafc", position: "relative" }} onBlur={(e) => { if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setCustomerSuggestOpen(false); }}><div style={{ fontWeight: 900, marginBottom: 8 }}>Khách hàng & kênh</div><div style={{ display: "grid", gap: 8 }}><div style={{ position: "relative" }}><input className="input-clean" placeholder="Gõ tên khách hàng" value={customerName} onFocus={() => setCustomerSuggestOpen(true)} onChange={(e) => { setCustomerName(e.target.value); setCustomerSuggestOpen(true); }} />{customerSuggestOpen && customerName.trim().length >= 2 ? <div style={customerSuggestBox}>{customerSuggestLoading ? <div style={customerSuggestEmpty}>Đang tìm khách hàng...</div> : customerSuggestions.length ? customerSuggestions.map((c) => <button key={c.id} type="button" style={customerSuggestItem} onMouseDown={(e) => { e.preventDefault(); selectCustomer(c); }}><b>{c.name}</b><span style={{ color: "#64748b" }}>{c.phone || "Chưa có SĐT"}{c.address ? ` • ${c.address}` : ""}</span></button>) : <div style={customerSuggestEmpty}>Không thấy khách cũ phù hợp. Khi lưu đơn sẽ tạo khách mới.</div>}</div> : null}</div><div style={{ position: "relative" }}><input className="input-clean" placeholder="Số điện thoại" value={customerPhone} onFocus={() => setCustomerSuggestOpen(true)} onChange={(e) => { setCustomerPhone(e.target.value); setCustomerSuggestOpen(true); }} />{customerSuggestOpen && customerName.trim().length < 2 && customerPhone.trim().length >= 2 ? <div style={customerSuggestBox}>{customerSuggestLoading ? <div style={customerSuggestEmpty}>Đang tìm khách hàng...</div> : customerSuggestions.length ? customerSuggestions.map((c) => <button key={c.id} type="button" style={customerSuggestItem} onMouseDown={(e) => { e.preventDefault(); selectCustomer(c); }}><b>{c.name}</b><span style={{ color: "#64748b" }}>{c.phone || "Chưa có SĐT"}{c.address ? ` • ${c.address}` : ""}</span></button>) : <div style={customerSuggestEmpty}>Không thấy SĐT khách cũ. Khi lưu đơn sẽ tạo khách mới.</div>}</div> : null}</div><select className="input-clean" value={saleChannel} onFocus={() => setCustomerSuggestOpen(false)} onChange={(e) => setSaleChannel(e.target.value)}><option value="">Chọn kênh bán</option>{saleChannels.map((c) => <option key={c} value={c}>{c}</option>)}<option value={CUSTOM_CHANNEL}>Khác, thêm mới</option></select>{saleChannel === CUSTOM_CHANNEL ? <input className="input-clean" placeholder="Tên kênh mới" value={customChannel} onFocus={() => setCustomerSuggestOpen(false)} onChange={(e) => setCustomChannel(e.target.value)} /> : null}<input className="input-clean" type="date" value={saleDate} onFocus={() => setCustomerSuggestOpen(false)} onChange={(e) => setSaleDate(e.target.value)} /></div></div>
    ...
    <section className="panel selected-goods-panel" style={{ padding: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 10 }}><div style={{ fontWeight: 900, fontSize: 18, color: "#075985" }}>Hàng hóa đã chọn</div><div style={{ color: "#0369a1", background: "#e0f2fe", border: "1px solid #7dd3fc", borderRadius: 999, padding: "5px 10px", fontWeight: 800 }}>Tổng số lượng: <b>{lines.length}</b></div></div>
